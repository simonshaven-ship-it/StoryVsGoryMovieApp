...
def cached_gemini_analysis(movie_title, gore_tolerance, puzzle_weight, pacing_weight):
    ...
    - Rule 1 (Agency): Tactical survival, hyper-competence, improvised blueprints (Add up to +2.5). MODERATE DEDUCTION (The 'Idiot Plot' Tax): Penalize films where characters make foolish decisions, BUT do NOT let secondary incompetence completely erase a lead protagonist's peak tactical survival agency.
    ...
    - Rule 4 (Narrative Puzzle): Intricate plot, high-concept narrative architecture, grand lore expansion (Add up to +2.0 * {puzzle_weight}). MASSIVE DEDUCTION: If a film relies on bizarre, unearned, or logically disconnected twists (e.g., absurd 'whodunit' reveals that make no sense), strip away points mercilessly.
    - Rule 5 (Pacing & Runtime Check): Deduct points (up to -2.0 * {pacing_weight}) for excessive runtime bloat, slow-burn wandering, or meandering travelogues that kill narrative momentum.
    ...
    - THE 'HIGH-CONCEPT LORE & SURVIVAL' PROTOCOL (THE 'PROMETHEUS' RULE): Films with massive conceptual ambition, grand philosophical lore puzzles, and iconic feats of peak individual survival agency (e.g., emergency DIY surgery) earn strong Rule 1 and Rule 4 bonuses. If secondary crew members commit 'Idiot Plot' blunders, the lead's elite agency and high-concept scope act as a safety net, ensuring the film's score rarely drops below the 7.2 - 7.8 range. However, if the film executes these concepts flawlessly without the blunders, it is fully authorized to score in the 9.0+ range.
    ...
    - THE 'SLOW SLASHER / PASSIVE SURVIVAL' TAX: If a movie is a slow-burn claustrophobic horror where the cast spends 80%+ of the runtime helpless, hiding, or reactively dying off without proactive counter-tactics (e.g., the original 1979 'Alien'), it gets minimal Rule 1 agency points, capping it strictly in the 5.5 - 6.5 range.
    - THE 'DEFIANT SACRIFICE & IMPROV' BENCHMARK (THE 'ALIEN 3' PROTOCOL): Unarmed tactical improvisation, squad coordination under dire conditions, and a climax where the protagonist asserts supreme agency through defiant self-sacrifice (e.g., 'Alien 3') MUST earn strong Rule 1 agency and Rule 3 payoff bonuses, guaranteeing a score in the 7.5 - 8.2 range.
    ...
    - THE HIGH-CONCEPT SATIRE EXEMPTION: Masterpieces of dark psychological satire bypass Rule 2 gore penalties completely, earning massive Rule 4 puzzle/satire bonuses (9.0 - 9.5).
